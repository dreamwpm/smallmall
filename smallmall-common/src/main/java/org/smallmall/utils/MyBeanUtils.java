...
public class MyBeanUtils extends PropertyUtilsBean {
	...
	public static Map<String, Object> convertBean2Map(Object obj, String[] ignores) {
		...
		List<Field> fieldList = findAllFieldsOfSelfAndSuperClass(obj.getClass());
		Field field = null;
		try {
		...
		try {
			...
			for (int i = 0; i < fs.length; i++) {
				...
				String type = fs[i].getGenericType().toString()
						.split("[.]")[fs[i].getGenericType().toString().split("[.]").length - 1];
				try {
					for (Method m : ms) {
						String temp = m.getName();
						if (temp.startsWith("set")) {
							String tmp = temp.substring(3, 4).toLowerCase() + temp.substring(4);
							if (igFirstChar(fieldname).equals(igFirstChar(tmp))) {
								String keyName = getAnnotationValue(fs[i].getAnnotations());
								if ("".equals(keyName)) {

									keyName = getAnnotationValue(
											clazz.getDeclaredMethod("get" + temp.substring(3), null).getAnnotations());
									if ("".equals(keyName)) {
										keyName = turnToTable(tmp, m.getAnnotations());
									}
								}
								...
								if (!keys.contains(keyName.toUpperCase())) {
									...
									for (String str : keys) {
										...
										if (temp_str.toLowerCase().equals(temp_field.toLowerCase())) {
											if (isMore > 0) {
								...
								if (!String.class.equals(paramType) && "".equals(val)) {
									continue;
								}
								...
								if ("String".equals(paramType.getSimpleName())) {
									m.invoke(obj, m.getParameterTypes()[0].getConstructor(String.class)
											.newInstance(val.toString()));
								} else if ("Double".equals(paramType.getSimpleName())
										|| "double".equals(paramType.getSimpleName())) {
									m.invoke(obj, Double.parseDouble(val.toString()));
								} else if ("Long".equals(paramType.getSimpleName())
										|| "long".equals(paramType.getSimpleName())) {
									m.invoke(obj, Long.parseLong(val.toString()));
								} else if ("Integer".equals(paramType.getSimpleName())
										|| "int".equals(paramType.getSimpleName())) {
									m.invoke(obj, Integer.parseInt(val.toString()));
								} else if ("BigInteger".equals(paramType.getSimpleName())) {
									m.invoke(obj, new BigInteger(val.toString()));
								} else if ("Date".equals(paramType.getSimpleName())) {
									m.invoke(obj, DateUtils.convertStringToDate(val.toString()));
								} else if ("Short".equals(paramType.getSimpleName())) {
									m.invoke(obj, Short.parseShort(val.toString()));
								} else if ("Byte".equals(paramType.getSimpleName())
										|| "byte".equals(paramType.getSimpleName())) {
									m.invoke(obj, Byte.parseByte(val.toString()));
								} else if ("char".equals(paramType.getSimpleName())) {
									if (val.toString().length() == 0) {
										continue;
									}
									m.invoke(obj, (char) (val.toString().charAt(0)));
								} else if ("Character".equals(paramType.getSimpleName())) {
									if (val.toString().length() == 0) {
										continue;
									}
									m.invoke(obj, (Character) (val.toString().charAt(0)));
								} else {
